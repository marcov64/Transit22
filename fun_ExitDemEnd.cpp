...
/*************************** OBSOLETE
        ...
        cur6 = SEARCH_CNDS(GRANDPARENT, "IdKFirm", v[59]);
        cur7 = SEARCHS(cur6, "BankK");
        
  		  
  		    INCRS(cur7,"KOandMRevenue",-(v[55]*v[96])/v[98]);
************************************************************/  		    
  		    if(V("ExitFlagEF")==1)
  		    {
        ...
        if(v[71]==0)
         INTERACTS(cur1, "Scrapping last KEN", v[55]);
  		    DELETE(cur1->hook);
  		    DELETE(cur1);
  		    //WRITES(cur1, "KillMe", 1);
  		    //INCRS(cur, "CashEF", v[20]*-1);
  		    //sprintf(msg, "\n Scrap KEF(%g)", v[20]); plog(msg);
...
if(v[0]!=0 && abs(v[0]-v[1])/v[0]>0.01)
 INTERACT("WRONG OD", v[1]);
...
CYCLE_SAFE(cur, "EnergyFirm")
 {
  ...
  v[38] = V("minSize");
  if( v[6]!=-1 && v[16]>v[18]) //don't kill new firms waiting for their first capital good to be delivered.
   {
    v[17]=VS(cur,"RRoKEF");
    v[7] = VS(cur, "CashEF");
...
if(abs(v[0]+v[20]-v[49])>10)
 INTERACT("WRONG assets", v[0]+v[20]-v[49]);
...
EQUATION("InvestmentDecisionEFXXX")
/*
Place an order of K if you need it and did not place an order as yet
*/
//V("DemographyEN");
...
if(v[11]!=0)
 { //not new entrant
	if(v[30]<=0)
	 ...
	 {//excess of energy demand in respect of capacity
    v[19]=V("msENCapacity");
		v[20]=v[30]*v[19];
		v[12]=V("MaxGrowthEF"); //
		v[21]=v[50]*v[12];
...
//  V("PlaceOrderEF");
  ...
  //RECALCS(cur,"PlaceOrderEF");
  v[50]=t;
  //INTERACT("InvestmentDecisionEF",v[50]);
	}
RESULT( 1)
...
if(abs(v[1]-v[75])/v[1]>0.000001)
 INTERACT("Wrong Deposits", v[75]);
